...
if [ ! -f PaddleClas/infer_static/AlexNet/__model__ ]; then
    ...
    wget https://sys-p0.bj.bcebos.com/Paddle-UnitTest-Model/PaddleClas.tgz --no-check-certificate
...
if [ ! -f PaddleDetection/infer_static/yolov3_darknet/__model__ ]; then
    ...
    wget https://sys-p0.bj.bcebos.com/Paddle-UnitTest-Model/PaddleDetection.tgz --no-check-certificate
...
if [ ! -f PaddleOCR/ch_ppocr_mobile_v1.1_cls_infer/model ]; then
    ...
    wget https://sys-p0.bj.bcebos.com/Paddle-UnitTest-Model/PaddleOCR.tgz --no-check-certificate
    tar -zxf PaddleOCR.tgz
fi

if [ ! -f PaddleSeg/infer_static/deeplabv3p/__model__ ]; then
    echo "==== Download PaddleSeg data and models ===="
    wget https://sys-p0.bj.bcebos.com/Paddle-UnitTest-Model/PaddleSeg.tgz --no-check-certificate
